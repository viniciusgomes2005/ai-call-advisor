...
class SpeechSegmenter:
    ...
    def push(self, item: AudioQueueItem) -> list[SpeechStarted | FinalizedAudio]:
        ...
        if self.in_speech:
            ...
            should_finalize = (
                (self.speech_ms >= self.min_speech_ms and self.trailing_silence_ms >= self.silence_end_ms)
                or utterance_ms >= self.max_utterance_ms
            )
            if should_finalize:
                events.append(self.finalize(item.source, item.sample_rate, self.timeline_seconds + chunk_seconds))

        self.timeline_seconds += chunk_seconds
        return events

    def flush(self, source: str, sample_rate: int = 16_000) -> FinalizedAudio | None:
        if not self.in_speech or not self.buffer:
            return None
        return self.finalize(source, sample_rate, self.timeline_seconds)

    def finalize(self, source: str, sample_rate: int, end_seconds: float) -> FinalizedAudio:
